# -*- coding: utf-8 -*-
"""
Created on Mon Aug 31 13:50:41 2020

@author: lenovo
"""
import numpy as np

def gauss_pt_wt(quadorder, qt, sdim):
    """
    ...
    if qt == 'GAUSS': 
        ...
        quadweight = np.zeros(quadorder**sdim)
        ...
        if quadorder == 1:
            ...
            print('unsupported integration order')
        ...
        if sdim == 1: 
            ...
            for i in range(1, quadorder+1):
                for j in range(1, quadorder+1):
                    quadpoint[n, :] = np.array([r1pt[i-1], r1pt[j-1]])           
                    quadweight[n] = r1wt[i-1]*r1wt[j-1] 
                    n = n + 1 # 下次迭代存放的位置
    Q = quadpoint
    W = quadweight 
    return W, Q
