# -*- coding: utf-8 -*-
"""
Created on Tue Sep  1 20:48:57 2020

@author: lenovo
"""
import numpy as np

def formm(kk,iel,stress):
    """
    ...
    m1 = 1/(3*(s_xx+s_yy+s_zz))*np.array([[1, 1, 0, 1], [1, 1, 0, 1],
                                          [0, 0, 0, 0], [1, 1, 0, 1]])

    m2 = 1/3*np.array([[2, -1, 0, -1], [-1,  2, 0, -1],
                       [0,  0, 6,  0], [-1, -1, 0,  2]])

    m3 = 1/3*np.array([[sx, sz, t_xy, sz], [sz, sy, t_xy, sx], 
                       [t_xy, t_xy, -3*sz, -2*t_xy],[sz, sx, -2*t_xy, sz]])
